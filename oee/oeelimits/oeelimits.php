    	...
    	var myData={getReasonsDetails:"getReasonsDetails"};
    	   $.ajax({
    	        ...
    	        success: function(obj){
    	        	...
    	        	globalOEEConfigData=obj.reasontDetails;
    	                    ...
    	                    { data: "message" },
    	      			    { data: "color_code",
    	      					render: function (data, type, row, meta) {
    	      						return '<input class="form-control" style="background-color:'+row.color_code+'" readonly>';
    	      					}
    	      				},
    	                    { data: "id" ,className: "text-left",
    	                      render: function (data, type, row, meta) {
    	                          var a='<button type="button" class="btn btn-primary btn-xs" onclick="tempData.oeelimitconfig.editReasons('+row.id+',\''+row.reason_type_id+'\');"><i class="fa fa-pencil-square-o"></i> </button>';
    	                          var b='<button type="button" class="btn btn-danger btn-xs" onclick="tempData.oeelimitconfig.deleteReason('+row.id+');"><i class="fa fa-trash"></i> </button>';
    	                         return a+' '+b;
    	                     }
    	                    },
    	                     ]
 ...
 saveReasons:function(){
		  ...
		  var message = $('#message').val();
		    if(reason_type_id == 0) { 
		    	...
		    	$('#msg').html('');
			    if($('#reason_code_no').val() == "") {
			        $('#reason_code_no').css('border-color', 'red');
			        return false;
			    }else{
		    ...
		    success: function(obj) {
		      if(obj.data !=null){
		        if(obj.data.infoRes=='S'){
		           ...
		           tempData.oeelimitconfig.loadAllReason();
...
editReasons:function (id, type_id){
    ...
    for(var i=0;i<globalOEEConfigData.length;i++){ 
       if(id==globalOEEConfigData[i].id){
         $('#reason_id').val(globalOEEConfigData[i].id);
         $('#message').val(globalOEEConfigData[i].message);
         $('#reason_type_id').val(type_id).change();
         $("#reason_code_no").val(globalOEEConfigData[i].id);
         $('#color').css('background-color',globalOEEConfigData[i].color_code);
         $('#color').val(globalOEEConfigData[i].color_code);
         break;
	  ...
	  $.ajax({
	    ...
	    success: function(obj) {
	      ...
	      if(obj.data !=null){
	        if(obj.data.infoRes=='S'){
	           $("#commonMsgRT").show();
	           $('#commonMsgRT').html('<p class="commonMsgSuccess"> <i class="fa fa-check"></i> '+obj.data.info+'</p>');
	           $('#fromReasonType')[0].reset();
	           $('#id_reason_type').val(''); 
	           tempData.oeelimitconfig.getReasonTypesForDropdown();

	        }else{
	          $("#commonMsgRT").show();
	           $('#commonMsgRT').html('<p class="commonMsgFail"> <i class="fa fa-warning"></i> '+obj.data.info+'</p>');
	        }  
	      } 
	      setTimeout(function(){  
	        $("#commonMsgRT").fadeToggle('slow');        
	      }, 1500);
	    }
	  });
  }
	  ...
	  $.ajax({
	    ...
	    success: function(obj) {
	      ...
	      if(obj.reasonTypes !=null){
	    	  ...
	    	  tempData.oeelimitconfig.loadReasonTypeTable(reasonTypeArray);
	    	 $("#reason_type_id").html('');
...
loadReasonTypeTable:function(reasonType){
    var DataTableProject = $('#rasonTypeTable').DataTable( {
        'paging'      : true,
        'lengthChange': false,
        'searching'   : true,
        'ordering'    : true,
        ...
        "columns": [
          ...
          { data: "id" ,className: "text-left",
              render: function (data, type, row, meta) {
                var a='<button type="button" class="btn btn-primary btn-xs" onclick="tempData.oeelimitconfig.editReasonType('+row.id+');"><i class="fa fa-pencil-square-o"></i> </button>';
...
editReasonType:function(id){
   for(var i=0;i<reasonTypeArray.length;i++){ 
       if(id==reasonTypeArray[i].id){
             $('#id_reason_type').val(id); 
             $('#reason_id1').val(id); 
             $('#color_code1').css('background-color',reasonTypeArray[i].color_code);
             $('#color_code1').val(reasonTypeArray[i].color_code); 
        	 $('#message1').val(reasonTypeArray[i].message);
		}
   }
},
gotoBack:function(){
        ...
        <form class="" id="fromReasons">  
          ...
          <input type="hidden" name="wc_id" id="wc_id"/>            
            <div class="form-group">
             ...
             <div class="row">
                <div class="col-md-6">
                   <label class="control-label col-md-4 col-sm-6 col-xs-12">Reason Code No. <span class="required">*</span></label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <input type="number" min="0" name="reason_code_no" id="reason_code_no"
                       placeholder="Reason Code Number" class="form-control"/>
                    </div>
                </div>
                <div class="col-md-6">
              ...
              <div class="row">
               <div id="msg" style="padding-left: 28px;color: red;"></div>
                   <div class="col-md-12 text-center">
                    <button type="button" id="addReason" onclick="tempData.oeelimitconfig.saveReasons();"  class="btn btn-sm btn-success">
                      <i class="fa fa-floppy-o"></i>&nbsp; Add Reason
                    </button>
                    <button type="button" id="updateReason" onclick="tempData.oeelimitconfig.saveReasons();"  class="btn btn-sm btn-success" style="display:none;">
    ...
    <div class="modal fade" id="reasonTypeModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <input type="hidden" name="id_reason_type" id="id_reason_type"/>  
          <div class="modal-body">          
               <div class="row">
                    <div class="col-md-6">
                      <label class="control-label col-md-4 col-sm-6 col-xs-12">Reason Id <span>*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" min="0" name="reason_id1" id="reason_id1" onkeyup=""
                           placeholder="Reason Id" class="form-control" />
                        </div>
                    </div>
                    <div class="col-md-6">
                      <label class="control-label col-md-4 col-sm-6 col-xs-12">Reason message<span>*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                    ...
                    <div class="col-md-6"  style="padding-top: 5px;">
                        <label class="control-label col-md-4 col-sm-6 col-xs-12">Color Code <span>*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                        	 <div class="col-md-8" style="padding-left: 0px; padding-right: 0px;">
                        		 <input class="form-control" id="color_code1" name="color_code1">
                        	</div>
                        	<div class="col-md-4" style="padding-left: 0px; padding-right: 0px; text-align: center;">
              ...
              <div class="" style="text-align: center;">
                <button type="button" class="btn btn-primary" onclick="tempData.oeelimitconfig.saveReasonType();">Save</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal" id="cancelBtn">Close</button>
              </div>
              <div id="commonMsgRT"></div>
               <br>
         </form>
            <div class="table-responsive"> 
              <table id="rasonTypeTable" class="table table-hover table-bordered  nowrap">
               <thead>
                 <tr>
                  <th>Reason Id</th>
                  <th>Message</th> 
