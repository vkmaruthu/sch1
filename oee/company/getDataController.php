...
if(isset($_POST['getPlantDetails'])){
    ...
    if($plant_id != ''){
        $plantQ="SELECT id,plant_code,plant_desc,address,contact_person,contact_number,image_file_name, comp_id FROM sfs_plant where id=".$plant_id." and comp_id=".$comp_id;
    }else{
...
if(isset($_POST['saveWC'])){
    ...
    $DataMarge=array('wc_code'=>$wc_code,
        'wc_desc'=>$wc_desc,
        'contact_person'=>$contact_person,
        'contact_number'=>$contact_number,
        'image_file_name'=>$filePathDB,
    ...
    if($wc_id == ''){
        ...
        if(!$res) {
            $error="WC Code Already Exists";
            $response['info']=$error;
            $response['infoRes']='E'; //Error
        }else {
            if(mysqli_errno() != 1062){
                move_uploaded_file($file_tmps,$filePath);
                $response['info']="WC Created Successfully";
                $response['infoRes']="S"; // success
                $response['mysqli_insert_id']=mysqli_insert_id($con);
            }else{
                $error="WC Code Already Exists";
...
if(isset($_POST['getWCDetails'])){
    ...
    $plant_id=$_POST['plant_id'];
    if( $wc_id != ''){
        $plantQ="SELECT id,wc_code,wc_desc,contact_person,contact_number,image_file_name, plant_id FROM sfs_workcenter where id=".$wc_id." and plant_id=".$plant_id;
    }else{
        $plantQ="SELECT id,wc_code,wc_desc,contact_person,contact_number,image_file_name, plant_id FROM sfs_workcenter where plant_id=".$plant_id;
    }
