...
editReasons:function (id, type_id){
    ...
    for(var i=0;i<globalCRData.length;i++){ 
       if(id==globalCRData[i].id){
         ...
         $("#reason_code_no").val(globalCRData[i].id);
	  ...
	  $.ajax({
	    ...
	    success: function(obj) {
	      ...
	      if(obj.reasonTypes !=null){
	    	  ...
	    	  tempData.oeecr.loadReasonTypeTable(reasonTypeArray);
	    	 $("#reason_type_id").html('');
...
loadReasonTypeTable:function(reasonType){
    var DataTableProject = $('#rasonTypeTable').DataTable( {
        ...
        "columns": [
          ...
          { data: "id" ,className: "text-left",
              render: function (data, type, row, meta) {
                var a='<button type="button" class="btn btn-primary btn-xs" onclick="tempData.oeecr.editReasonType('+row.id+');"><i class="fa fa-pencil-square-o"></i> </button>';
